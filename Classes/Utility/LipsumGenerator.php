<?php

declare(strict_types=1);

namespace Supseven\ThemeDev\Utility;

/**
 * A utility class for generating Lorem Ipsum text.
 *
 * inspired by the vhs lipsum viewhelper
 *
 * @author Volker Kemeter <v.kemeter@supseven.at>
 */
class LipsumGenerator
{
    /**
     * Generates a string containing a specified number of paragraphs with randomized word order.
     *
     * @param int $paragraphCount The number of paragraphs to generate. Defaults to 1.
     * @param int|null $wordsPerParagraph The number of words per paragraph. If null, a random number between 5 and 20 is used.
     * @return string|array<string> A string of generated paragraphs, with words in random order.
     */
    public function generate(int $paragraphCount = 1, ?int $wordsPerParagraph = null, bool $asArray = false): string|array
    {
        $lipsum = $this->getDefaultLoremIpsum();
        $paragraphs = array_values(array_filter(explode("\n", $lipsum), static fn ($paragraph) => !empty($paragraph)));
        $paragraphs = array_slice($paragraphs, 0, $paragraphCount);
        $wordsPerParagraph ??= random_int(5, 20);

        foreach ($paragraphs as $index => $paragraph) {
            $length = $wordsPerParagraph;
            $words = explode(' ', $paragraph);
            shuffle($words);
            $paragraphs[$index] = implode(' ', array_slice($words, 0, $length));
        }

        return !$asArray ? implode("\n", $paragraphs) : $paragraphs;
    }

    /**
     * Retrieves the default Lorem Ipsum string.
     *
     * @return string The default Lorem Ipsum text.
     */
    protected function getDefaultLoremIpsum(): string
    {
        $lipsum = '';

        return gzuncompress(base64_decode($lipsum));
    }
}
